...
import {
  ...
  SidebarAddon,
  SidebarPublisher,
} from "renderer/components/App/SideBar";
...
const releaseCache = new DataCache<AddonVersion[]>(
  "releases",
  1000 * 3600 * 24
);
 ...
 * @param addon
 */
export const getAddonReleases = async (
  addon: Addon
  ...
  try {
    ...
    releases.forEach((version, index) => {
      ...
      if (currentVersionTitle[1] !== otherVersionTitle[1]) {
        ...
        currentVersionTitle[5] !== otherVersionTitle[5] &&
        index === releases.length - 1
      ) {
        releases[index].type = "minor";
...
export const fetchLatestVersionNames = async (addon: Addon): Promise<void> => {
  addon.tracks.forEach(async (track) => {
    const trackLatestVersionName = await AddonData.latestVersionForTrack(
      addon,
      track
    );
...
const App: React.FC<{ configuration: Configuration }> = ({ configuration }) => {
  ...
  const initialSelectedItem = (): string => {
    ...
    if (
      cachedItem === "settings" ||
      cachedItem === "debug" ||
      addons.find((x) => x.key === cachedItem)
    ) {
      return cachedItem;
    } else {
      return addons[0].key;
  ...
  switch (selectedItem) {
    ...
    default:
      ...
      sectionToShow = (
        <AircraftSection
          key={selectedItem}
          addon={addons.find((x) => x.key === selectedItem)}
          publisher={configuration.publishers.find((x) =>
            x.addons.includes(addons.find((x) => x.key === selectedItem))
          )}
        />
      );
      break;
                ...
                <div className="h-full flex flex-col divide-y divide-gray-700">
                          ...
                          overriddenByAddon={() => {
                            try {
                              return configuration.publishers
                                .find((publisher) =>
                                  publisher.addons.find(
                                    (item) =>
                                      item.overrideAddonWhileHidden ===
                                      addon.key
                                  )
                                )
                                .addons.find(
                                  (item) =>
                                    item.overrideAddonWhileHidden === addon.key
                                );
                            } catch (e) {
                              return null;
                            }
                  ...
                  <div className="mt-auto">
                    {process.env.NODE_ENV === "development" && (
                      <SidebarItem
                        iSelected={selectedItem === "debug"}
                        onClick={() => setSelectedItem("debug")}
                      >
                    ...
                    <SidebarItem
                      iSelected={selectedItem === "settings"}
